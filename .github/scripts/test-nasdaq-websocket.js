/**
 * Nasdaq Streaming WebSocket Health Check
 *
 * Connects to /nasdaq, subscribes to AAPL for SNAPSHOT+TICK, and passes when
 * the server confirms the handshake. The /nasdaq handler bridges to an upstream
 * market-data feed, so the strongest *server-controlled* signal is the
 * `connected` event (sent right after auth + plan gate succeed). We also accept
 * `subscription_status: subscribed` and any well-formed `market_data` push as
 * bonus pass signals.
 *
...
const WS_URL = `wss://ws.prixe.io/nasdaq?api_key=${process.env.PRIXE_PRO_PLUS_API_KEY}`;
...
function validateConnected(message) {
  try {
    ...
    if (parsed.data?.status !== 'connected') return null;
    return { valid: true, plan: parsed.data?.plan };
  } catch (e) {
    return null;
  }
}
...
function validateSubscriptionStatus(message) {
  try {
    ...
    return { valid: true, symbol: parsed.data?.symbol };
  } catch (e) {
    return null;
  }
}
...
function validateMarketData(message) {
  try {
    ...
    const d = parsed.data;
    if (!d || !d.topic || !d.symbol || typeof d.payload !== 'object') return null;
    return { valid: true, topic: d.topic, symbol: d.symbol };
  } catch (e) {
    return null;
  }
}
...
function detectServerError(message) {
  try {
    ...
    if (parsed.event === 'error') {
      return parsed.data?.message || 'unknown server error';
    }
...
async function testWebSocket() {
  ...
  return new Promise((resolve, reject) => {
    console.log('Connecting to Nasdaq WebSocket...');
    ...
    const timeout = setTimeout(() => {
      ws.close();
      if (!passed) {
        reject(new Error('Nasdaq WebSocket test timed out after ' + TIMEOUT_MS + 'ms'));
      }
    }, TIMEOUT_MS);
    ...
    ws.on('message', (data) => {
      ...
      if (connected?.valid) {
        ...
        // Primary pass signal: server-controlled `connected` event.
        // Send subscribe as a deeper check; any of subscription_status or market_data
        // would be bonus, but `connected` alone is sufficient to mark the service up.
        if (!subscribeSent) {
          subscribeSent = true;
        ...
        done('Nasdaq WebSocket test passed - `connected` event received');
        return;
      }

      const sub = validateSubscriptionStatus(message);
      if (sub?.valid) {
        console.log('Subscription confirmed for symbol:', sub.symbol);
        done('Nasdaq WebSocket test passed - subscribed to ' + sub.symbol);
        return;
      }

      const md = validateMarketData(message);
      if (md?.valid) {
        console.log('market_data received: topic=' + md.topic + ' symbol=' + md.symbol);
        done('Nasdaq WebSocket test passed - received market_data push (' + md.topic + ')');
        return;
      }
    });
    ...
    ws.on('error', (error) => {
      clearTimeout(timeout);
      reject(new Error('Nasdaq WebSocket error: ' + error.message));
    });
    ...
    ws.on('close', (code, reason) => {
      if (!passed) {
        ...
        reject(new Error('Nasdaq WebSocket closed before handshake: ' + code + ' ' + reasonStr));
      }
    });
  });
}
...
function writeResultFile(status, responseTime, code) {
  const outPath = process.env.NASDAQ_WS_RESULT_FILE;
  if (!outPath) return;
  const fs = require('fs');
